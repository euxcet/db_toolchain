...
class TrajectoryDetector(Detector):
  def __init__(self, name:str, input_streams:dict[str, str], output_streams:dict[str, str], checkpoint_file:str,
               imu_window_length:int, execute_interval:int, timestamp_step:float, move_threshold:float):
    super(TrajectoryDetector, self).__init__(
      name=name, model=TrajectoryLSTMModel(), input_streams=input_streams,
      output_streams=output_streams, checkpoint_file=checkpoint_file)
    self.touching = False
  ...
  def handle_input_stream_imu(self, data:IMUData, timestamp:float) -> None:
    ...
    if self.counter.count() and self.imu_window.full() and self.touching:
      ...
      self.output(self.OUTPUT_STREAM_RESULT, move)
  ...
  def handle_input_stream_touch_state(self, data:str, timestamp:float) -> None:
    if data == 'touch_down':
      ...
      if self.last_unstable_move.capacity() > 0:
        self.output(self.OUTPUT_STREAM_RESULT, -self.last_unstable_move.sum())
        self.last_unstable_move.clear()
