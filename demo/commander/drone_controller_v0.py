...
class DroneController(Node):
  ...
  def perform_action(self, action: DroneAction):
    print(action)
    timestamp = time.time()
    # if self.last_action in [DroneAction.MOVE_BACKWARD, DroneAction.MOVE_FORWARD] and \
    ...
    if timestamp - self.last_action_timestamp > 1.5 or \
      ...
      if action == DroneAction.LAND:
        ...
        self.output(self.OUTPUT_EDGE_COMMAND, 'rc -20 0 0 0')
      elif action == DroneAction.MOVE_RIGHT:
        self.output(self.OUTPUT_EDGE_COMMAND, 'rc 20 0 0 0')
      elif action == DroneAction.MOVE_UP:
        self.output(self.OUTPUT_EDGE_COMMAND, 'rc 0 0 -20 0')
  ...
  def handle_input_edge_touch(self, data: int, timestamp: float) -> None:
    if data == 6:
      self.perform_action(DroneAction.MOVE_LEFT)
    elif data == 7:
      self.perform_action(DroneAction.MOVE_RIGHT)
    elif data == 11:
      self.perform_action(DroneAction.MOVE_DOWN)
    elif data == 12:
      self.perform_action(DroneAction.MOVE_UP)
